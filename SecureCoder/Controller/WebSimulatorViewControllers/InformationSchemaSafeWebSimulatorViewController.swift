...
class InformationSchemaSafeWebSimulatorViewController: WebSimulatorViewController {
...
"""),
            ...
            GuideText(text: "このauthorに対応するプレースホルダにバインドした値は攻撃者が渡した以下の値でした。\n' UNION SELECT table_name, column_name, data_type FROM information_schema.columns;--", programingLanguages: [.sql]),
            GuideText(text: "この攻撃者が渡した値を設定するときには既にSQL文のコンパイルは終了しているので、体験編の時のようにSQL文の構造が変更されることはなく、booksテーブルからauthorが「' UNION SELECT table_name, column_name, data_type FROM information_schema.columns;--」の行を探しにいきます。", programingLanguages: [.sql]),
            GuideText(text: "今回のbooksテーブルには前述のとおり、author列が「' UNION SELECT table_name, column_name, data_type FROM information_schema.columns;--」の行はありませんので、今回の検索結果では何も行が取得されません。", programingLanguages: [.sql]),
            GuideText(text: "この結果、この赤枠で囲まれた部分には<table>要素のヘッダー部分しか表示されていません。", programingLanguages: [.html]),
            GuideText(text: "これでDBMS内部情報の漏洩を防ぐことができました。")
            ], onEnter: { completion in
